// Node-side identity + storage. Same shape as the MCP wrapper.
...
function keyPath() {
  ...
  return join(homedir(), '.falldns', 'api-identity.json');
}
...
export async function loadOrCreateIdentity() {
  ...
  return {
    did,
    async sign(msg) {
      const sig = await subtle.sign({ name: 'ECDSA', hash: 'SHA-256' }, privateKey, new TextEncoder().encode(msg));
    ...
    async verify(msg, signature, verifyingDid) {
      ...
      try {
        ...
        return await subtle.verify({ name: 'ECDSA', hash: 'SHA-256' }, publicKey, sig, new TextEncoder().encode(msg));
      } catch { return false; }
    }
  };
}
...
export function fileStorage(path) {
  const p = path || join(homedir(), '.falldns', 'api-records.json');
